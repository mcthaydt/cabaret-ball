...
func before_each() -> void:
	...
	# Disable persistence for test isolation (avoid loading user://savegame.json)
...
func test_scene_trigger_component_has_area3d_collision() -> void:
	...
	# When: Resolving trigger area via helper (ensures component finished setup)
	var area: Area3D = trigger.get_trigger_area()

	# Then: Component should have Area3D for collision detection and parent it under the entity
	assert_not_null(area, "Component should create TriggerArea child")
	assert_true(area is Area3D, "TriggerArea should be Area3D")
	assert_true(entity.get_node_or_null("TriggerArea") == area,
		"TriggerArea should be parented on the entity for transform inheritance")
...
func test_full_scene_load_exterior_to_interior() -> void:
	...
	if not FileAccess.file_exists("res://scenes/gameplay/gameplay_exterior.tscn"):
		pending("scenes/gameplay/gameplay_exterior.tscn must exist - see MANUAL_SCENE_CREATION_GUIDE.md")
		return
	if not FileAccess.file_exists("res://scenes/gameplay/gameplay_interior_house.tscn"):
		pending("scenes/gameplay/gameplay_interior_house.tscn must exist - see MANUAL_SCENE_CREATION_GUIDE.md")
		return
	...
	var exterior_scene: Node = load("res://scenes/gameplay/gameplay_exterior.tscn").instantiate()
...
func test_full_scene_load_interior_to_exterior() -> void:
	...
	var interior_scene: Node = load("res://scenes/gameplay/gameplay_interior_house.tscn").instantiate()
...
func test_spawn_point_restoration_after_door_transition() -> void:
	...
	# Then: Find player and spawn point in new scene
	...
	assert_lt(distance, 0.5, "Player should be positioned near spawn marker (distance < 0.5)")
